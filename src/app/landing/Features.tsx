...
const FEATURES = [
  ...
  { Icon: Trophy, title: "XP & Levels", desc: "Earn XP for every session. Level up from Seedling to Grandmaster. 7 ranks to conquer.", color: "#D9A441" },
...
export default function Features() {
  ...
  return (
    <section ref={sectionRef} className="py-24 px-6" style={{ background: "#F4EEDB" }}>
        ...
        <div ref={gridRef} className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {FEATURES.map(f => (
            <div key={f.title} className="rounded-2xl p-5 transition-all hover:-translate-y-1 hover:shadow-lg"
              style={{ background: "#FDF9F0", border: "1px solid rgba(0,0,0,0.05)", boxShadow: "0 1px 8px rgba(0,0,0,0.02)" }}>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center mb-3" style={{ background: `${f.color}12` }}>
                <f.Icon size={20} style={{ color: f.color }} />
              </div>
              <h3 className="text-[16px] font-extrabold mb-1.5" style={{ color: "#3D2E24", fontFamily: "var(--font-baloo)" }}>{f.title}</h3>
              <p className="text-[13px] leading-relaxed font-medium" style={{ color: "#6B5D52" }}>{f.desc}</p>
            </div>
          ))}
        </div>
      </div>
