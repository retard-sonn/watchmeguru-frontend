...
const STEPS = [
  ...
  { num: "3", icon: "🔥", title: "Study, earn, grow", desc: "Complete sessions, upload proof, earn XP. Your learning tree grows with every streak.", color: "#D9A441" },
];
        ...
        <div ref={cardsRef} className="grid md:grid-cols-3 gap-6">
          {STEPS.map(s => (
            <div key={s.num}
              ...
              <h3 className="text-[18px] font-extrabold mb-2" style={{ color: "#3D2E24", fontFamily: "var(--font-baloo)" }}>{s.title}</h3>
              <p className="text-[14px] leading-relaxed font-medium" style={{ color: "#6B5D52" }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
